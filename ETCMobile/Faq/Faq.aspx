...
<div class="search"><label for="select"></label>
    <asp:DropDownList ID="ddl1" runat="server" AppendDataBoundItems="True" DataSourceID="sds_FaqCatgry_List"
        ...
        <asp:ListItem Value="%">請選擇分類</asp:ListItem>
    ...
    <asp:GridView ID="GridView1" runat="server" DataSourceID="sds_Faq_List"
        ...
        PageSize="10" EmptyDataText="目前沒有資料，或您的查詢沒有資料。">
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="問題" SortExpression="Subject">
                ...
                <ItemTemplate><asp:Label ID="lblIndex" runat="server" CssClass ="color14592f" Visible="false" ></asp:Label>
                    <span class="color000000"><asp:LinkButton ID="lbtn1" runat="server" ></asp:LinkButton></span> 
                </ItemTemplate>
                <ItemStyle Width="80%" />
            </asp:TemplateField>
...
<div align="right" class="scott" id="LitPage"><asp:Literal ID="ltlPager" runat="server"></asp:Literal></div> 
    ...
    <asp:SqlDataSource ID="sds_Faq_List" runat="server" ConnectionString="<%$ ConnectionStrings:AdminModuleConnectionString %>"
        SelectCommand="SELECT Faq.PublicID, Faq.Subject, Faq.Content, FaqCatgry.CateGoryName, Faq.PublishDate, Faq.PublishExpireDate FROM Faq INNER JOIN FaqCatgry ON Faq.CateGoryID = FaqCatgry.CateGoryID INNER JOIN SitesApRelation ON FaqCatgry.CateGoryID = SitesApRelation.ApUID WHERE (Faq.Subject LIKE N'%' + @Keyword + N'%') AND (CONVERT (VARCHAR(5), Faq.CateGoryID) LIKE @CateGoryID) AND (SitesApRelation.SiteID = @SiteID) AND (SitesApRelation.ApKeyword = 'Faq') AND ((DATEDIFF(yyyy, PublishDate, GETDATE()) >= 0) AND (DATEDIFF(yyyy, GETDATE(), PublishExpireDate) >= 0) AND (DATEDIFF(dy, PublishDate, GETDATE()) >= 0) AND (DATEDIFF(dy, GETDATE(), PublishExpireDate) >= 0)) ORDER BY Faq.PublishDate DESC, FaqCatgry.CateGoryID, Faq.PublicID DESC">
        <SelectParameters>
