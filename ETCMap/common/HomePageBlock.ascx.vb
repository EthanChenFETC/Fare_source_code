...
''' </summary>
''' <remarks></remarks>
Partial Class common_HomePageBlock
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If System.Configuration.ConfigurationManager.AppSettings("BlockWordLimit") IsNot Nothing Then
            _newsWordLimit = CInt(System.Configuration.ConfigurationManager.AppSettings("BlockWordLimit").ToString)
        End If
        If _CallMode = False Then
        End If
        Dim SiteID As Integer = CInt(System.Configuration.ConfigurationManager.AppSettings("SiteID")).ToString
        getGroupID(SiteID)
        Me.btnMore.PostBackUrl = "~/Publish.aspx?cnhb=" & _GroupID '要加回去
        Me.sds_Important_News.SelectParameters("SiteID").DefaultValue = SiteID
    End Sub
    ''' <summary>
    ''' 政策公告-頭條專區
    ''' </summary>
            ...
            Try
                If dr IsNot Nothing Then
                    If dr.Read Then
                        ...
                        Me.sds_Important_News.SelectParameters("GroupID").DefaultValue = _GroupID
                        Me.sds_Important_News.SelectParameters("Qty").DefaultValue = CInt(dr("ItemCount"))
                    End If
                End If
    ...
    Protected Sub Repeater1_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles Repeater1.ItemDataBound
        If e.Item.ItemType = ListItemType.AlternatingItem OrElse e.Item.ItemType = ListItemType.Item Then
            ...
            Dim Subject As String = ModuleMisc.LimitWord(dr.Item("Subject").ToString, _newsWordLimit)
            HyperLink1.Text = RemoveXSS(Subject)
            ...
            '發佈日期
            Dim lblDate As Literal = CType(e.Item.FindControl("lblDate"), Literal)
            Dim PublishDate As Date = CType(dr.Item("PublishDate"), Date)
            'Dim PublishDate As Date = CType(dr.Item("UpdateDateTime"), Date)
            'Dim PublishDate As Date = CType(dr.Item("UpdateDateTime"), Date)

            lblDate.Text = ModuleMisc.GetChineseDate(PublishDate).ToString("yyy-MM-dd", System.Globalization.DateTimeFormatInfo.InvariantInfo)

            '顯示New的圖形
            Dim ImageNew As Image = CType(e.Item.FindControl("ImageNew"), Image)
            If DateDiff(DateInterval.Day, PublishDate, Date.Today) > _newsNewDate Then
                ImageNew.Visible = False
            Else
                ImageNew.ImageUrl = Page.ResolveUrl("~/images/new_icon.gif")
            End If
